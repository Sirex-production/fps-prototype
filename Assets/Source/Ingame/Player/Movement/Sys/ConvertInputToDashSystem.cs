...
namespace Ingame.Player.Movement
{
	public sealed class ConvertInputToDashSystem : IExecuteSystem
	{
		...
		public void Execute()
		{
			...
			if(!inputCmp.dashInput || inputCmp.moveInput.sqrMagnitude < .0001f || dashingCmp.timePassedSinceLastDash < _playerConfig.DashCooldown)
				return;

			var playerTransform = playerEntity.transformMdl.transform;
			var dashVelocity = playerTransform.forward * inputCmp.moveInput.y + playerTransform.right * inputCmp.moveInput.x;
			dashVelocity.y = 0;
